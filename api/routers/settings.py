...
class ServiceConnectionConfig(BaseModel):
    ...
    api_key_or_token: str = Field(
        ..., description="API key or authentication token", min_length=1
    )
...
async def get_all_settings(orchestrator=Depends(get_orchestrator)):
    ...
    return AllServicesConfigResponse(
        ...
        ) if settings.sabnzbd_enabled else None,
        ...
        ) if settings.sonarr_enabled else None,
        ...
        ) if settings.radarr_enabled else None,
        plex=ServiceConnectionConfigResponse(
            ...
            status=plex_status,
        ) if settings.plex_enabled else None,
    )
