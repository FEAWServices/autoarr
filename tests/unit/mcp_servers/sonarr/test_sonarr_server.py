...
class TestSonarrMCPServerToolExecution:
    ...
    async def test_get_episodes_tool_execution(
        ...
        mock_sonarr_client.get_episodes.assert_called_once_with(
            series_id=1, season_number=None
        )
    ...
    @pytest.mark.asyncio
    async def test_delete_series_tool_execution(
...
class TestSonarrMCPServerErrorHandling:
    ...
    @pytest.mark.asyncio
    async def test_handles_invalid_parameter_type(
        self, sonarr_server, mock_sonarr_client
    ) -> None:
    ...
    async def test_handles_network_error(
        ...
        mock_sonarr_client.get_series.side_effect = SonarrConnectionError(
            "Connection failed"
        )
