...
async def sonarr_integration_client(
    ...
    client = SonarrClient(
        url=sonarr_base_url,
        api_key=sonarr_integration_api_key,
        timeout=30.0
    )
    yield client
    await client.close()
...
class TestSonarrSeriesWorkflow:
    ...
    async def test_full_series_lifecycle(
        ...
        search_results = [
            sonarr_series_factory(series_id=0, title="Breaking Bad", tvdb_id=81189)
        ]
        ...
        await sonarr_integration_client.delete_series(
            series_id=series_id,
            delete_files=True
        )
    ...
    async def test_episode_search_and_monitoring_workflow(
        ...
        retrieved_episodes = await sonarr_integration_client.get_episodes(
            series_id=series_id
        )
...
class TestSonarrDataIntegrity:
    ...
    async def test_preserves_complex_series_structure(
        ...
        complex_series = sonarr_series_factory(
            series_id=1,
    ...
    async def test_handles_unicode_and_special_characters(
        ...
        series_with_unicode = sonarr_series_factory(
            series_id=1,
            title="Café: L'Été (2020) – Part 1 & 2"
        )
        httpx_mock.add_response(json=[series_with_unicode])

        result = await sonarr_integration_client.search_series(
            term="Café: L'Été"
        )
...
class TestSonarrPerformance:
    ...
    async def test_handles_large_response_sets(
        ...
        large_series_list = [
            sonarr_series_factory(series_id=i, title=f"Series {i}")
...
class TestSonarrCalendarAndQueue:
    ...
    async def test_calendar_date_range_filtering(
        ...
        calendar_data = sonarr_calendar_factory(
            days=7,
            episodes_per_day=2,
