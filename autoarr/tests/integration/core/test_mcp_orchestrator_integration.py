...
class TestOrchestratorRealServerConnections:
    ...
    @pytest.mark.asyncio
    @pytest.mark.integration
    async def test_connection_pool_with_real_servers(self, orchestrator):
...
class TestOrchestratorRealToolExecution:
    ...
    async def test_sabnzbd_get_history(self, orchestrator):
        ...
        # Act
        result = await orchestrator.call_tool("sabnzbd", "get_history", {"start": 0, "limit": 10})
    ...
    async def test_sonarr_get_calendar(self, orchestrator):
        ...
        result = await orchestrator.call_tool(
            "sonarr", "get_calendar", {"start": start_date, "end": end_date}
        )
    ...
    async def test_radarr_search_movies(self, orchestrator):
        ...
        # Act
        result = await orchestrator.call_tool("radarr", "search_movies", {"term": "Inception"})
    ...
    @pytest.mark.asyncio
    @pytest.mark.integration
    async def test_tool_execution_with_complex_params(self, orchestrator):
        """Test tool execution with complex nested parameters."""
        # Act
        result = await orchestrator.call_tool(
            "sonarr",
            "search_series",
            {"term": "Breaking Bad", "limit": 5, "filters": {"year": 2008, "network": "AMC"}},
        )
    ...
    @pytest.mark.asyncio
    @pytest.mark.integration
    async def test_tool_execution_returns_metadata(self, orchestrator):
    ...
    @pytest.mark.asyncio
    @pytest.mark.integration
    async def test_sequential_tool_calls_maintain_state(self, orchestrator):
...
class TestOrchestratorMultiServerCoordination:
    ...
    @pytest.mark.asyncio
    @pytest.mark.integration
    async def test_aggregate_data_from_multiple_servers(self, orchestrator):
    ...
    async def test_cross_server_workflow(self, orchestrator):
        ...
        history = await orchestrator.call_tool("sabnzbd", "get_history", {"limit": 5})
    ...
    @pytest.mark.asyncio
    @pytest.mark.integration
    async def test_partial_failure_graceful_degradation(self, orchestrator):
...
class TestOrchestratorRealErrorRecovery:
    ...
    @pytest.mark.asyncio
    @pytest.mark.integration
    async def test_retry_on_transient_server_error(self, orchestrator):
    ...
    @pytest.mark.asyncio
    @pytest.mark.integration
    async def test_continue_operation_with_degraded_service(self, orchestrator):
...
class TestOrchestratorPerformance:
    ...
    async def test_high_throughput_tool_calls(self, orchestrator):
        ...
        tasks = [orchestrator.call_tool("sabnzbd", "get_version", {}) for _ in range(100)]
    ...
    async def test_memory_usage_under_load(self, orchestrator):
        ...
        for batch in range(10):
            tasks = [orchestrator.call_tool("sabnzbd", "get_queue", {}) for _ in range(50)]
