...
export const ContentCard = ({ content, onAdd }: ContentCardProps) => {
  ...
  const posterUrl = content.posterPath ? `${TMDB_IMAGE_BASE_URL}${content.posterPath}` : null;
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-start justify-between gap-2 mb-2">
          <div className="flex-1 min-w-0">
            <h3 className="text-lg font-semibold text-text-primary truncate">{content.title}</h3>
        ...
        {/* Overview */}
        <div data-testid="content-overview" className="text-sm text-text-secondary leading-relaxed">
        ...
        {content.originalTitle && content.originalTitle !== content.title && (
          <p className="text-xs text-text-muted mt-2">Original: {content.originalTitle}</p>
        )}
      </div>
    </div>
  );
